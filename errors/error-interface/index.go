...
func sendSMSToCouple(msgToCustomer, msgToSpouse string) (int, error) {
	// ?
	customerMsgCost, customerMsgErr := sendSMS(msgToCustomer)
	spouseMsgCost, spouseMsgErr := sendSMS(msgToSpouse)

	if customerMsgErr != nil {
		return 0, customerMsgErr
	}

	if spouseMsgErr != nil {
		return 0, spouseMsgErr
	}

	return customerMsgCost + spouseMsgCost, nil
}
